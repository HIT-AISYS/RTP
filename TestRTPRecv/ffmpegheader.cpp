...
AVPacket *VideoEncoder::getEncodePacket(AVFrame *srcFrame)
{
    ...
    int64_t videoPts = av_rescale_q(av_gettime() - startTime, AVRational{1, AV_TIME_BASE}, videoEnCodecCtx->time_base);
...
AVPacket *AudioEncoder::getEncodePacket(AVFrame *srcFrame)
{
    ...
    //添加时间戳 实时时间戳方式
    int64_t audioPts = av_rescale_q(av_gettime() - startTime, AVRational{1, AV_TIME_BASE}, audioEnCodecCtx->time_base);
